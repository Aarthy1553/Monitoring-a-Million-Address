...
# Create the subscription query to monitor DEXTrades
...
# Function to monitor DEXTrades for specified addresses
async def monitor_dex_trades():
    ...
    try:
        ...
        async for result in transport.subscribe(query, variable_values={"addresses": addresses}):
            if result.data:
                print("DEXTrade Data:", result.data)  # Debugging output
                dex_trade_data = pd.json_normalize(result.data["Solana"]["DEXTrades"])

                # Select only the relevant fields: Name, Symbol, MintAddress, and average
